...
class DataLoader():
    def __init__(self, prefix):
        ...
        for rel_id, data_matrix in self.dl.links['data'].items():
            # Get subset belonging to entities in relation
            start_i = self.dl.nodes['shift'][relations[rel_id].entities[0].id]
            end_i = start_i + self.dl.nodes['count'][relations[rel_id].entities[0].id]
            start_j = self.dl.nodes['shift'][relations[rel_id].entities[1].id]
            end_j = start_j + self.dl.nodes['count'][relations[rel_id].entities[1].id]
            rel_matrix = data_matrix[start_i:end_i, start_j:end_j]
            self.data[rel_id] = SparseMatrix.from_scipy_sparse(rel_matrix.tocoo())

        for ent_id, attr_matrix in self.dl.nodes['attr'].items():
            if attr_matrix is None:
                # Attribute for each node is a single 1
                attr_matrix = np.ones(self.dl.nodes['count'][ent_id])[:, None]
            n_channels = attr_matrix.shape[1]
            rel_id = ent_id + num_relations
            n_instances = self.dl.nodes['count'][ent_id]
            indices = torch.arange(n_instances).unsqueeze(0).repeat(2, 1)
            self.data[rel_id] = SparseMatrix(
                indices = indices,
                values = attr_matrix,
    ...
    def get_node_classification_data(self):
        ...
        target_entity = next(iter(self.dl.labels_train['count'].keys()))
        self.schema_out = DataSchema([entities[target_entity]],
                                [Relation(0, 
                                          [entities[target_entity],
                                           entities[target_entity]],
                                           is_set=True)])
        self.data_target = SparseMatrixData(self.schema_out)
        labels = np.zeros((self.dl.nodes['count'][0],
                           self.dl.labels_train['num_classes']), dtype=int)
        val_ratio = 0.2
        train_idx = np.nonzero(self.dl.labels_train['mask'])[0]
