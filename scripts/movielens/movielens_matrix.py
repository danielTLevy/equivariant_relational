...
for relation in relations:
    ...
    print(relation_name)
    if relation.is_set:
        relation_to_matrix = set_relation_to_matrix
    else:
        relation_to_matrix = binary_relation_to_matrix
...
data_target[0] = SparseMatrix(
                    indices=torch.arange(ent_users.n_instances,
                                         dtype=torch.int64).repeat(2,1),
                    values=torch.zeros([ent_users.n_instances, output_classes]),
                    shape=(ent_users.n_instances, ent_users.n_instances,
                           output_classes))
data = data.to(device)
