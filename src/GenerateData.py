...
class SchoolGenerator():
    ...
    def generate_embeddings(self, n_dim_ent=5):
        np.random.seed(0)
        embed_students = np.random.normal(size=(self.n_student, n_dim_ent))
        embed_courses = np.random.normal(size=(self.n_course, n_dim_ent))
        embed_professors = np.random.normal(size=(self.n_professor, n_dim_ent))
        self.embeddings = [embed_students, embed_courses, embed_professors]

    def generate_data(self, n_dim_ent=5):
        ...
        for relation in self.schema.relations:
            ...
            ent_embeddings  = [self.embeddings[ent.id] for ent in entities]
            relation_data = rel_fns[relation.id](*ent_embeddings)
            data[relation.id] = torch.Tensor(relation_data).unsqueeze(0)
        return data
