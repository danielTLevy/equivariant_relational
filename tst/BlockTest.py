...
def test_layer_single_block(X, entities, relation_i, relation_j, in_dim, out_dim):
    torch.manual_seed(0)
    ...
    print("X_out: ", X_out)
    expected_shape = [out_dim] + relation_j.get_shape()
...
if __name__ == '__main__': 
    ...
    test_layer_single_block(X, entities, relation_i, relation_j, 1, 7)
    ...
    test_layer_single_block(X, entities, relation_i, relation_j, 1, 3)
    ...
    test_layer_single_block(X, entities, relation_i, relation_j, 1, 1)
    ...
    c = 5
    d = 2
    X = torch.Tensor(np.arange(b*a*d*a*d*d, dtype=np.float32)).view(1, b, a, d, a, d, d)
    ...
    # %%
    # Example 5
    # Repeat of example 2 but with 3 channels going to 5
    X = torch.tensor(np.arange(48, dtype=np.float32)).view(3,4,4)
    entities = [Entity(0, 3), Entity(1, 2), Entity(2, 4)]
    relation_i = Relation(0, [entities[2], entities[2]])
    relation_j = Relation(1, [entities[0], entities[1], entities[1]])
    
    test_layer_single_block(X, entities, relation_i, relation_j, 3, 5)
