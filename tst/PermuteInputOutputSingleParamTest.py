...
if __name__ == '__main__': 
    ...
    X = torch.tensor(np.arange(12)).view(1, 1,2,2,3)
    ...
    X = torch.tensor(np.arange(16)).view(1, 1, 4,4)
    output_shape = (1, 1, 3, 2, 2)
    ...
    X = torch.tensor(np.arange(18)).view(1, 1, 3, 2, 3)
    mapping = [({2,4},{4}), ({3},set()), (set(),{2,3})]
    output_shape = (1, 1, 4, 4, 3)
    entities = [Entity(0, 3), Entity(1, 2), Entity(2, 4)]
    ...
    output_shape = (1, 1, d, b, c)
    assert test_param_with_permutation(X, mapping, output_shape, entities, relation_i, relation_j)
    
    
    #%%
    # Example 5:
    # Same as example 2 but with multiple channels and batches
    X = torch.tensor(np.arange(96)).view(2, 3, 4,4)
